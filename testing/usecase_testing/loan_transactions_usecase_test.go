...
func (suite *LoanTransactionUseCaseTestSuite) TestFIndLoanTransactionByUserIdAndTrxId_Success() {
	userId := "1"
	trxId := "1"
	suite.trxrm.Mock.On("GetByUserIdAndTrxId", mock.Anything, mock.Anything).Return(dto.LoanTransactionResponseDto{}, nil)
	actual, err := suite.trxu.FIndLoanTransactionByUserIdAndTrxId(userId, trxId)
	assert.Nil(suite.T(), err)
	assert.Equal(suite.T(), dto.LoanTransactionResponseDto{}, actual)
}

func (suite *LoanTransactionUseCaseTestSuite) TestFindAllLoanTransaction_Success() {
	suite.trxrm.Mock.On("GetAll", mock.Anything, mock.Anything).Return([]dto.LoanTransactionResponseDto{}, nil)
	pageMock := 1
	limitMock := 1
	actual, err := suite.trxu.FindAllLoanTransaction(pageMock, limitMock)
	assert.Nil(suite.T(), err)
	assert.Equal(suite.T(), []dto.LoanTransactionResponseDto{}, actual)
}
...
func (suite *LoanTransactionUseCaseTestSuite) TestFindByUserId_Success() {
	userId := "1"
	suite.trxrm.Mock.On("GetByUserID", mock.Anything).Return([]dto.LoanTransactionResponseDto{}, nil)
...
func (suite *LoanTransactionUseCaseTestSuite) TestFindById_Success() {
	id := "1"
	suite.trxrm.Mock.On("GetByID", mock.Anything).Return(dto.LoanTransactionResponseDto{}, nil)
