...
func (suite *LoanTransactionControllerTestSuite) TestGetLoanTransacttionByUserIdAndTrxIdHandler_Success() {
	suite.lum.Mock.On("FIndLoanTransactionByUserIdAndTrxId", mock.Anything, mock.Anything).Return(dto.LoanTransactionResponseDto{}, nil)
...
func (suite *LoanTransactionControllerTestSuite) TestGetAllHandlerHandler_Success() {
	suite.lum.Mock.On("FindAllLoanTransaction", mock.Anything, mock.Anything).Return([]dto.LoanTransactionResponseDto{}, nil)
	suite.controller.Router()
	record := httptest.NewRecorder()
	req, err := http.NewRequest(http.MethodGet, "/api/v1/transactions/loans?page=1&limit=1", bytes.NewBuffer([]byte{}))
	assert.NoError(suite.T(), err)
	ctx, _ := gin.CreateTestContext(record)
	ctx.Request = req
...
func (suite *LoanTransactionControllerTestSuite) TestGetHandlerByUserId_Success() {
	suite.lum.Mock.On("FindByUserId", mock.Anything).Return([]dto.LoanTransactionResponseDto{}, nil)
	suite.controller.Router()
	record := httptest.NewRecorder()
...
func (suite *LoanTransactionControllerTestSuite) TestGetHandlerById_Success() {
	suite.lum.Mock.On("FindById", mock.Anything).Return(dto.LoanTransactionResponseDto{}, nil)
	suite.controller.Router()
	record := httptest.NewRecorder()
