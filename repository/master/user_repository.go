...
func (u *userRepository) CreateUser(payload model.User) (model.User, error) {
	var user model.User
	err := u.db.QueryRow(rawquery.CreateUser, payload.Username, payload.Email, payload.Password, payload.Role, payload.CreditScore, time.Now(), time.Now()).Scan(&user.Id, &user.Username, &user.Email, &user.Password, &user.Role, &user.CreditScore, &user.CreatedAt, &user.UpdatedAt)
...
func (u *userRepository) GetUserById(id string) (model.User, error) {
	var user model.User
	err := u.db.QueryRow(rawquery.GetUserById, id).Scan(&user.Id, &user.Username, &user.Email, &user.Role, &user.CreditScore, &user.CreatedAt, &user.UpdatedAt)
...
func (u *userRepository) GetUserByUsernameOrEmailAndPassword(usernameOrEmail, password string) (model.User, error) {
	var user model.User
	err := u.db.QueryRow(rawquery.GetUserByUsernameOrEmailAndPassword, usernameOrEmail, password).Scan(&user.Id, &user.Username, &user.Email, &user.Role, &user.CreditScore, &user.CreatedAt, &user.UpdatedAt)
...
func (u *userRepository) GetAllUsers() ([]model.User, error) {
    rows, err := u.db.Query(rawquery.GetAllUsers)
    if err != nil {
        return nil, err
    }
    defer rows.Close()
...
func (u *userRepository) UpdateUser(id string, payload model.User) error {
	_, err := u.db.Exec(rawquery.UpdateUser, id, payload.Username, payload.Email, payload.Password, payload.Role, payload.CreditScore, time.Now())
